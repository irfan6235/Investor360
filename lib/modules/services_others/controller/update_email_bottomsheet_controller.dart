...
class UpdateEmailBottomsheetController extends GetxController {
  ...
  var displayEmail = ''.obs; // Initialized with an empty string
  RxString emaill = ''.obs;

  @override
  Future<void> onInit() async {
    selectFirstItem();
    getEcasDetailsApi("IN487875", "12055001");
    super.onInit();
  }

  // void fetchEmail() async {
  //   var prefs = await SharedPreferences.getInstance();
  //   var eemail = prefs.getString(KeyConstants.email);
  //   if (eemail == null || eemail.isEmpty) {
  //     displayEmail.value =
  //         accountDematList.value.emailId ?? ''; // Default empty string
  //   } else {
  //     displayEmail.value = eemail;
  //   }
  // }
...
/*        accountDematList = dashboardController.responseData.value!.dematAccountList!
  ...
  Future<void> updateEmailAPI(
      String dpid, String clientId, String email) async {
    UpdateEcasEmailRequest updateemailrequest = UpdateEcasEmailRequest();
    SharedPreferences prefs = await SharedPreferences.getInstance();
    var sessionId = prefs.getString(KeyConstants.sessionId);
    var pan = prefs.getString(KeyConstants.pan);
    updateemailrequest.channelId = "Device";
    updateemailrequest.deviceId = await getDeviceId();
    updateemailrequest.deviceOS = getDeviceOS();
    updateemailrequest.sessionId = sessionId;
    updateemailrequest.clientid = clientId;
    updateemailrequest.dpId = dpid;
    updateemailrequest.email = email;
    updateemailrequest.pan = pan;
    updateemailrequest.signCS =
        getSignChecksum(updateemailrequest.toString(), "");

    try {
      isLoading.value = true;
      var res = await serviceOtherRepo.updateEcasEmail(updateemailrequest);

      if (res != null && res.statusCode == 200) {
        Map<String, dynamic> mainJson;
        if (res.data is String) {
          showSnackBar(
              Get.context!, "You Have Successful Updated Your Email ID");
          getEcasDetailsApi("IN487875", "12055001");
      ...
      if (res != null && res.statusCode == 200) {
        ...
        if (data != null && data is Map<String, dynamic>) {
          ...
          if (isSubscribed && email != null && email.isNotEmpty) {
            ...
            emaill.value = accountDematList.value.emailId;
          }
        } else {
