...
class CmrCoontroller extends GetxController {
  final ServiceReportRepo serviceReportRepo = ServiceReportRepo();
  RxList<GetCmrListResponseItem> reportItems = RxList<GetCmrListResponseItem>();
  var isLoading = false.obs;

  var isDownloaded = false.obs;

  TextEditingController textEditingControllerBeneidCMR =
  ...
  Future<void> setInitialDpIdAndClientId() async {
    if (dashboardController.responseData.value != null) {
      ...
      if (dematAccountList != null) {
        ...
        textEditingControllerBeneidCMR.text =
            selectedDpId.value + "-" + selectedClientId.value;
      }
    }
  }

  Future<void> getCmrListAPI(String dpid, String clientId) async {
    try {
      isLoading.value = true;

      // Fetch session ID from SharedPreferences
      SharedPreferences prefs = await SharedPreferences.getInstance();
      var sessionId = prefs.getString(KeyConstants.sessionId);

      // Construct the request object
      Getcmrlistrequest getCmrListRequest = Getcmrlistrequest(
        channelId: "Device",
      ...
      getCmrListRequest.signCS =
          getSignChecksum(getCmrListRequest.toString(), "");

      // Make the API call
      var res = await serviceReportRepo.getCmrList(getCmrListRequest);
      print(res);

      // Handle the response
      if (res != null && res.statusCode == 200) {
        // Parse the common base response
        CommonBaseResponse response = CommonBaseResponse.fromJson(res.data);
        var body = res.data;

        if (response.responseCode == "0") {
          // Parse the specific response body
          GetCmrListResponse getCmrListResponse =
              GetCmrListResponse.fromJson(body);
          List<GetCmrListResponseItem> fetchedItems =
              getCmrListResponse.data ?? [];
  ...
  Future<void> createCMrFileAPI(BuildContext context) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    var sessionId = prefs.getString(KeyConstants.sessionId);

    CreateCmrFileRequest createCmrFileRequest = CreateCmrFileRequest(
      channelId: "Device",
      deviceId: await getDeviceId(),
      deviceOS: getDeviceOS(),
      sessionId: sessionId,
    ...
    createCmrFileRequest.signCS =
        getSignChecksum(createCmrFileRequest.toString(), "");

    try {
      var res = await serviceReportRepo.createCmrFile(createCmrFileRequest);
      print(res);
...
/*  Future<void> getCmrFileAPI({required String fileId}) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    var sessionId = prefs.getString(KeyConstants.sessionId);

    GetCmrFileRequest getCmrFileRequest =
        GetCmrFileRequest(sessionId: sessionId!, fileId: fileId);

    try {
      PopUpLoading.onLoading(Get.context!);
      var res = await serviceReportRepo.getCmrFile(getCmrFileRequest);
      print(res);
  ...
  Future<void> getCmrFileAPI({required String fileId}) async {
    ...
    GetCmrFileRequest getCmrFileRequest = GetCmrFileRequest(
      channelId: "Device",
      deviceId: await getDeviceId(),
      deviceOS: getDeviceOS(),
      sessionId: sessionId!,
    ...
    getCmrFileRequest.signCS =
        getSignChecksum(getCmrFileRequest.toString(), "");
    try {
      PopUpLoading.onLoading(Get.context!);
      var res = await serviceReportRepo.getCmrFile(getCmrFileRequest);
      print(res);
...
class BottomSheetCMRController extends GetxController {
...
class CMRBottomSheet extends StatelessWidget {
  final BottomSheetCMRController controller =
      Get.put(BottomSheetCMRController());
  final CmrCoontroller cmrCoontroller = Get.find();
  ...
  Widget build(BuildContext context) {
    ...
    if (controller.selectedIndex.value == null &&
        cmrCoontroller.dashboardController.responseData.value != null &&
        cmrCoontroller
                .dashboardController.responseData.value!.dematAccountList !=
            null &&
        cmrCoontroller.dashboardController.responseData.value!.dematAccountList!
            .isNotEmpty) {
      var firstAccount = cmrCoontroller
          .dashboardController.responseData.value!.dematAccountList!.first;
      cmrCoontroller.filterHoldingsByDPIDAndClientID(
          ...
          return Container(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                Obx(() {
                  if (cmrCoontroller.dashboardController.responseData.value ==
                          null ||
                      cmrCoontroller.dashboardController.responseData.value!
                              .dematAccountList ==
                          null) {
                    return const Center(child: CircularProgressIndicator());
                  }

                  var demartAccountList = cmrCoontroller.dashboardController
                        ...
                        return Column(
                          children: [
                            ListTile(
                              ...
                              onTap: () {
                                cmrCoontroller.selectedDpId.value =
                                    holdingData.dpId!;
                                cmrCoontroller.selectedClientId.value =
                                    holdingData.clientId!.toString();
                                cmrCoontroller.filterHoldingsByDPIDAndClientID(
                                  holdingData.dpId!,
                                  holdingData.clientId!,
                                );
                                cmrCoontroller.getCmrListAPI(
                                    cmrCoontroller.selectedDpId.value
                                        .toString(),
                                    cmrCoontroller.selectedClientId.value
                                        .toString());
